...
Components are functions which return something "renderable".

To let babel know that a function we created should be interpreted as a function
reference in React.creatElement type, not a simple string type ("div", "span",
etc) we'll need to start the name with uppercase.

PropTypes is a dev feature only. React will not run PropTypes it in production
builds because it is not good for performance. Babel will remove the code for
us. No need to use if we already use Typescript.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=netanella1%40gmail.com).
