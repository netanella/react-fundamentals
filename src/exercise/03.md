...
JSX is not JavaScript, it's just a more "fun" syntax that gets compiled to JS
using code compilers like Babel.

Interpolation is taking one type of thing inside another thing. Ex. using the $
sign in template literals (going from string land -> js land) or using the <script/> tag
(from html land -> js land).

When using curly braces {} we are telling babel that this is js code that
doesn't need to be touched. It will look exactly the same after compiling.

We can use only expressions (evaluates to a value), not statements (imperative
logic). The reason is that it doesn't make sense to put a statement as an
argument in "createElement" function call.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=03%3A%20Using%20JSX&em=netanella1%40gmail.com).
