...
When submitting a `<form>`, the browser by default adds the values as a query in
the URL, and we get a page refresh. We can avoid this by setting
`event.preventDefault()`.

Synthetic events - React creates those for performance reasons. to access the
actual event we can use `event.nativeEvent`.

`console.dir` - see the DOM element's properties.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=netanella1%40gmail.com).
