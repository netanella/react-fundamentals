...
When generating elements by looping over a list, we're interpolating an array of
renderable elements. When the list is re-rendered, React doesn't know whether
you added/removed an item in the middle of the array, or if you added/removed
from the end and updated the existing elements values.

This can be a problem if any of those React elements represent a component that
has state.

When considering the index as identifier, when an element is removed in index
[n], for example, it will behave like the element in index [n] was updated by
the properties of the element in index [n+1]. The meaning is that the state of
the removed element would remain.

That's why we give React a key, so it can associate the old React elements with
the new ones.

Using "index" as the key won't help in those cases because that's the default
behaviour (just like no key at all).

Keys can also be used to intentionally reset the state of a component.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=07%3A%20Rendering%20Arrays&em=netanella1%40gmail.com).
